...
$stmt->bindValue(':u_pw', $lpw_h, PDO::PARAM_STR);  //Integer（数値の場合 PDO::PARAM_INT)
$stmt->bindValue(':email', $_SESSION['mail'], PDO::PARAM_STR);  //Integer（数値の場合 PDO::PARAM_INT)
...
$sql_s = "UPDATE tkn_pre_table SET flag=1 WHERE mail=:email";
$stmt_s = $pdo->prepare($sql_s);
$stmt_s->bindValue(':email', $_SESSION['mail'], PDO::PARAM_STR); 
...
}else if($status_s==false){
  $error_s = $stmt_s->errorInfo();
  exit("QueryError2:".$error_s[2]);
